...
import quizDataEN from '../dttqenl.json';
...
return (
    ...
    <div className="text-center mb-8 md:mb-12">
        <div className="bg-gray-100 p-4 pt-2 sm:p-6 flex flex-col sm:flex-row items-center justify-center">  
            ...
            <button onClick={handleSignUp} className="bg-yellow-500 ml-2 mr-2 text-white mt-2 mb-2 py-2 px-4 rounded block w-full sm:w-auto transition-colors duration-300 ease-in-out hover:bg-yellow-600">
                {language === 'CN' ? '注册会员' : 'Sign Up for Full Experience'}
            </button>
            <button onClick={handleLogIn} className="bg-green-500 ml-2 mr-2 text-white py-2 px-4 rounded block w-full sm:w-auto transition-colors duration-300 ease-in-out hover:bg-green-600">
                {language === 'CN' ? '会员登录' : 'Member Login'}
            </button>
        </div>
    </div>
    <p className="text-gray-600 italic mt-10">
                {language === 'CN' ? '以上为游客体验内容,了解更多请加微信：zdyq740 ' : 'Above is the guest experience content'}
    </p>
    <p className="text-sm text-gray-600  italic">
        开通会员模式可以获得完整792道双语题库，刷新页面仍可保存记录错题库，答题位置，后期更会更新模拟考试分类练习等相关功能
    </p>
</div>
