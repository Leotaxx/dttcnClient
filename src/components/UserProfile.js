...
const UserProfile = () => {
    ...
    useEffect(() => {
        const questions = JSON.parse(localStorage.getItem('incorrectQuestions')) || [];
    ...
    const clearMockTestHistory = () => {
        ...
        setResults([]);
        alert(language === 'CN' ? '模拟测试记录已成功清除。' : 'Mock test history cleared successfully.');
    };
    ...
    const texts = {
        ...
        clearHistory: language === 'CN' ? '清空模拟测试记录' : 'Clear Mock Test History',
    ...
    return (
        <div className="flex flex-col justify-between h-screen bg-green-100">
            ...
            {language === 'CN' ? 
                <>全题库练习包含792道题，有内建逻辑可以记录答题进度，错题，在全题库练习中可直接通过该页面的错题练习按钮进入错题练习，答对的错题将会从错题库中移除。 <br /> 模拟测试为40分钟模考，实际考试时间为45分钟，所有模考记录将会显示在下方。</> : 
                ...
                <button onClick={toggleLanguage} className="mb-2 bg-blue-300 px-3 py-1 rounded-full text-sm md:text-base shadow-sm">
                    {texts.toggleLanguage}
                </button>
                <button className="w-full bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 mb-2 rounded focus:outline-none focus:shadow-outline" onClick={() => navigateToSection('allquiz')}>
                    {texts.allQuiz}
                </button>
                <button className="w-full bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 mb-2 rounded focus:outline-none focus:shadow-outline" onClick={() => navigateToSection('mocktest')}>
                    {texts.mockTest}
                </button>
                <button className="w-full bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onClick={clearMockTestHistory}>
                    {texts.clearHistory}
                </button>
            ...
            <div className="mock-test-results flex-grow  mt-2">
                ...
                {results.map((result, index) => (
                    <div key={index} className="bg-yellow-200  hover:bg-yellow-300 mb-1 mt-1 p-1 rounded-lg shadow cursor-pointer" onClick={() => fetchQuestionDetails(result.incorrectMockIds)}>
                        {language==='CN'?<p>时间: {result.date}, 分数: {result.score} / {result.total}</p>:<p>Date: {result.date}, Score: {result.score} / {result.total}</p>}
                    </div>
                ))}
            </div>
            </div>
                {isModalOpen && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
                        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
                            ...
                            <h4 className='bg-green-300'>{language === 'CN' ? '下滑查看完整问题详解' : 'Incorrect Questions Details'}</h4>
                            
                            </div>
                            {selectedResultDetails.map((detail, index) => (
                                <div key={index} className="mb-4 p-4 rounded-lg bg-gray-100 shadow">
                                    <p>{detail.question_text}</p>
                                    {detail['image_name'] !== "loader.gif" && (
                                        <img src={getImagePath(detail['image_name'])} alt="Quiz Illustration" className="w-2/3 max-w-sm mx-auto mb-4" />
                                    )}
                                    <p>{`正确答案: ${detail[`option_${detail.correct_answer.toLowerCase()}`]}`}</p>
                                </div>
                            ))}
